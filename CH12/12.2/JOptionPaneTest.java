...
public class JOptionPaneTest
{
	...
	public void init()
	{
		...
		messageTypePanel = new ButtonPanel("选择消息的类型" , 
			new String[]{"WARNING_MESSAAGE" , "QUESTION_MESSAGE" , "PLAIN_MESSAGE"});
		...
		bottom.setBorder(new TitledBorder(new EtchedBorder() , "弹出不同的对话框" , TitledBorder.CENTER ,
			TitledBorder.TOP));
		...
		//创建用于弹出输入对话框的Panel
		inputPanel = new ButtonPanel("输入对话框" , new String[]{"当行文本框" , "下拉列表选择框"});
		inputBn.addActionListener(new ShowAction());
		inputPanel.add(inputBn);
		//创建用于弹出选择对话框的Panel
		optionsPanel = new ButtonPanel("选择对话框" , new String[]{"字符串选项" , "图标选项" , "对象选项"});
			...
			switch(event.getActionCommand())
			{
				...
				case "选择对话框":
					JOptionPane.showOptionDialog(jf , getMessage() , 
						"选择对话框" , getOptionType() , getDialogType() ,
							null , getOptions() , "a");
					break;
			}
		}
...
//JRadioButton空间，切Panel扩展类可以置顶一个字符串作为TitledBorder
