...
public class ImageViewer
{
	...
	public void init()
	{
		...
		chooser.setAcceptAllFileFilterUsed(false);		//1
		//为稳健选择器指定自定义的FileView对象
		chooser.setFileView(new FileIconView(filter));
		//为文件选择器指定一个预览图片的附件
		chooser.setAccessory(accessory);	//2
		//设置与篮球图片组件的大小和边框
		...
		openItem.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				//如果用户选择了APPROVE(同意)按钮，机打开，保存的等效按钮
...
class ExtensionFileFilter extends FileFilter
{
	...
	//继承Fileter类必须实现的抽象方法，返回该文佳过滤器是否接受该文件
