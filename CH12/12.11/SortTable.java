...
public class SortTable
{
	...
	//将原表格里的model包装秤新的SorttableTableModel对象
	SortableTableModel sorterModel = new SortableTableModel(table.getModel());
	public void init()
	{
		//使用包装后的SorttableTableModel对象作为JTable的model对象
		table.setModel(sorterModel);
		//为每列的列头增加鼠标监听器
		table.getTableHeader().addMouseListener(new MouseAdapter()
		{
			public void mouseClicked(MouseEvent event)		//126
			{
				...
				//将JTable中的列索引转换成对应的TableModel中的列索引
...
class SortableTableModel extends AbstractTableModel
{
	...
	private Row[] rows;
	//将一个已经存在的TableModel对象封装成SortableTableModel对象
	...
	//下面方法实现把该model的方法委托给原分装的model来实现
